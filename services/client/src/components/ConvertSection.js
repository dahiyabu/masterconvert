...
export default function ConvertSection({ API_URL }) {
  ...
  const fetchFormatsAndLimits = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    if (fingerprint) {  // Ensure fingerprint is available before calling fetchAccountLimits
      fetchAccountLimits();
      fetchFormatsAndLimits();
    }
  }, [fingerprint,fetchAccountLimits,fetchFormatsAndLimits]);  // Dependency on fingerprint
...
*/
  ...
  const handleConvert = async () => {
    ...
    if (!validFiles.length || !fingerprint) return;
